...
import { Network, X, ZoomIn, ZoomOut, Move, ChevronRight, ChevronLeft } from 'lucide-react';
...
const MindMap = ({ file }: { file: FileData }) => {
  ...
  useEffect(() => {
    const fetchMindMap = async () => {
      ...
      try {
        ...
        // Add type assertion to res.data
        const data = res.data as { nodes: { id: string; label: string; }[]; edges: MindMapEdge[] };
  ...
  const createZoomBehavior = () => {
    return d3.zoom<SVGSVGElement, unknown>()
      .scaleExtent([0.5, 2])
      .on('zoom', (event) => {
        // Only update the transform, don't trigger transitions
  ...
  const renderMindMap = (visibleNodes: MindMapNode[], visibleEdges: MindMapEdge[]) => {
    ...
    // Draw links with morph transition
    ...
    // Morph the links to their final positions
    links.transition()
      .duration(800)
      .delay((d, i) => i * 20)
      .attr('d', d => {
    ...
    // Draw nodes with morph transition
    ...
    // Morph nodes to their final positions and sizes
    nodeGroups.transition()
      ...
      .attr('opacity', 1)
      .on('end', function(d) {
        // Add click event after transition
    ...
    // Animate rectangles to final size
  ...
  }, [nodes, edges, loading, currentTransform]);
